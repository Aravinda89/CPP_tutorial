...
**
*/
void Matrix::setGrid(std::vector< std::vector<float> > initial_vector){
	grid = initial_vector;
	rows = initial_vector.size();
	cols = initial_vector[0].size();
}
...
std::vector<float>::size_type Marix::getRows(){
	return rows;
}
